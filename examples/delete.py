...
def on_message(message): # Fires when a message is sent to any of the connected channels.
    if message.has_me and not "moderator/1" in message.badges: # Checks if the user used /me and was not a moderator.
        bot.delete(message.channel, message.id) # Delete the message with unique ID.
        ...
        if warnings[message.user] > 3:
            bot.timeout(message.channel, message.user, 20 * warnings[message.user], "Stop using /me in chat!")
        
        if warnings[message.user] > 20:
            bot.ban(message.channel, message.user, "You used /me too many times.")
